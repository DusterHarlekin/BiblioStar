    ...
    if (count($filtered_get)) { // not empty
        ...
        if (count($dates) && $dateQuery) {
            ...
            if (count($dates) > 1) {
                ...
                $query .= " STR_TO_DATE(" . $dateQuery . ", '%d/%m/%Y') BETWEEN STR_TO_DATE('" . $dateValues[0] . "', '%d/%m/%Y') AND STR_TO_DATE('" . $dateValues[0] . "', '%d/%m/%Y') OR STR_TO_DATE(" . $dateQuery . ", '%d/%c/%Y') BETWEEN STR_TO_DATE('" . $dateValues[0] . "', '%d/%c/%Y') AND STR_TO_DATE('" . $dateValues[0] . "', '%d/%c/%Y') OR DATE_FORMAT(STR_TO_DATE(" . $dateQuery . ", '%Y%m%d'), '%d/%m/%Y') BETWEEN '" . $dateValues[0] . "' AND '" . $dateValues[0] . "'";
            }
        }
    }
