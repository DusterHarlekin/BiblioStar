<template>
  <q-page class="q-px-lg q-py-md">
    <div class="row q-gutter-sm items-center q-mb-md">
      ...
      <q-icon name="mdi-home-search" size="md" color="grey-9" />
      <div class="text-h5 text-weight-medium font-title text-grey-9">
        Sala "{{ currentRoom.cod_sala }}"
      </div>
    </div>
          ...
          <q-list>
            <q-item>
              <q-item-section>
                ...
                <q-item-label>{{ currentRoom.cod_sala }}</q-item-label>
              </q-item-section>
            </q-item>
            ...
            <q-item>
              <q-item-section>
                ...
                <q-item-label>{{ currentRoom.DESCRIPCION }}</q-item-label>
              </q-item-section>
            </q-item>
          </q-list>
        </div>
...
const fetchRoom = async () => {
  ...
  const url =
    process.env.API_URL +
    `salas.php?N=${roomId}&session_user_name=${localStorage.getItem(
      "usuario"
  ...
  currentRoom.value = clearSpaces(data[0]);
};
